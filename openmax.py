...
def non_member_class_distance(logits, mean_logits):
    distances = []
    for logit in logits:
        member_class_indices = list(np.where(torch.sigmoid(torch.tensor(logit)) > 0.5)[0])
        logit_distances = []
...
class OpenMaxPredictor(inference_utils.MultilabelPredictor):
    ...
    def confidences_to_predicted_labels(
        ...
        for k in self.class_list:
            ...
            curr_class_nearest_distances = np.min(self.distance_function(logits, class_logits), axis=1)
            distances += [curr_class_nearest_distances]
